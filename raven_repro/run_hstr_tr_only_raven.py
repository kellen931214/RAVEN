#!/usr/bin/env python3
"""Run RAVEN against a completed official-math HSTR TR-only cohort.
...
TR_ONLY_PROFILE = "official_math_tr_only"
TR_ONLY_PROTOCOL = "hstr_tr_only_sfwmark_ablation_paired_direct_generation"
...
def validated_manifest(source_dir: Path) -> tuple[Path, dict[str, Any]]:
    ...
    required = {
        "method": "HSTR",
        "profile_name": TR_ONLY_PROFILE,
        "pattern_variant": "tr_only",
        "score_mode": "center_channel_3_complex_l1",
        "scheduler_type": "DDIM",
    }
    for field, expected in required.items():
        if manifest.get(field) != expected:
            raise ValueError(f"bundle manifest {field}={manifest.get(field)!r}, expected {expected!r}")
    if manifest.get("watermark_channels") != [3] or manifest.get("heterogeneous_channels") != []:
        raise ValueError("TR-only bundle must contain only watermark channel 3 and no heterogeneous channels")
...
def hstr_mask_sha256(bundle_dir: Path, manifest: dict[str, Any]) -> str:
    ...
    if provider.profile != TR_ONLY_PROFILE or provider.score_definition != "hstr_tr_only_score=-channel_3_l1":
        raise ValueError("loaded bundle is not the channel-3-only HSTR profile")
...
def build_metadata(rows: list[dict[str, Any]], source_dir: Path, bundle_dir: Path, manifest: dict[str, Any], metadata_path: Path) -> None:
    ...
    with metadata_path.open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writeheader()
        for source in rows:
            if source.get("profile_name") != TR_ONLY_PROFILE or source.get("protocol") != TR_ONLY_PROTOCOL:
                raise ValueError(f"sample {source.get('sample_name')}: source provenance is not HSTR TR-only direct generation")
            writer.writerow({
                ...
                "hstr_protocol_mode": TR_ONLY_PROTOCOL,
...
def run_watermarked_attack(args: argparse.Namespace, metadata_path: Path) -> None:
    command = [
        ...
        "--dataset", "hstr_tr_only_direct_1001", "--method", "HSTR",
...
def add_untouched_clean_records(rows: list[dict[str, Any]], source_dir: Path, output_dir: Path, metadata_path: Path) -> None:
    ...
    for source in rows:
        ...
        write_record(output_dir, "clean", run_id, {
            "run_id": run_id, "role": "clean", "dataset": "hstr_tr_only_direct_1001",
