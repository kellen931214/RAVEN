...
This module remains an independent re-derivation on purpose: it is the second
opinion, not the authority. The authoritative fixtures are now generated by
executing the official source itself — see ``tests/official_sfwmark_source.py``,
``tests/fixtures/hsqr_official_fixtures.json`` and
``tests/test_hsqr_official_fixtures.py``. Every digest below has since been
confirmed bitwise against that official execution for keys 0, 1, 1024 and 2047,
so the two derivations agree.

Inversion parity is covered separately by
``tests/test_hsqr_inversion_parity.py`` and
