...
import hstr_official_reference as official
...
class OfficialPatternTests(unittest.TestCase):
    ...
    def test_selected_patterns_match_frozen_reference(self):
        for index in (0, 1, 13):
            provider = HSTRProvider(**provider_kwargs(hstr_key_index=index))
            expected = official.make_pattern(OFFICIAL_BASE_KEY_SEED + index)
            self.assertTrue(torch.equal(provider.gt_patch.cpu(), expected), index)
    ...
    def test_injected_latent_scores_as_watermark_with_per_image_scores(self):
        ...
        self.assertNotEqual(hstr_bundle.tensor_sha256(clean_a), hstr_bundle.tensor_sha256(clean_b))
...
class BundleTests(unittest.TestCase):
    def test_bundle_round_trip_and_threshold_binding(self):
        with TemporaryDirectory() as tmp:
            ...
            artifact = hstr_bundle.build_threshold_artifact(
                threshold=-0.5,
                binding=reloaded.bundle.binding_config(),
                target_fpr=0.01,
                empirical_fpr=0.0,
                empirical_tpr=1.0,
                roc_auc=1.0,
    ...
    def test_incompatible_bundle_fails_closed(self):
        with TemporaryDirectory() as tmp:
            ...
            with self.assertRaises(HstrBundleError):
...
class RuntimeTests(unittest.TestCase):
    def test_roc_uses_higher_score_and_strict_fpr(self):
        roc = hstr_runtime.official_roc([0.9, 0.8, 0.7], [0.1, 0.2, 0.3], 0.01)
    ...
    def test_corrupt_image_is_error_not_negative_detection(self):
        with TemporaryDirectory() as tmp:
            ...
            row = hstr_runtime.score_image(provider, None, corrupt, {"threshold": -1.0, "comparison_operator": ">=", "score_direction": "higher_is_watermarked"})
    ...
    def test_pairing_from_generation_sidecars(self):
        with TemporaryDirectory() as tmp:
            ...
            pairing = hstr_runtime.resolve_pairing([pos / "000000.png"], [neg / "000000.png"])
