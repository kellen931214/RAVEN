...
from utils.wm import hstr_bundle, hstr_runtime
from utils.wm.hstr_bundle import HstrBundle, HstrBundleError
...
def _apply_hstr_bundle_defaults(args, argv):
    ...
    manifest = hstr_bundle.read_json(Path(args.hstr_bundle_dir) / hstr_bundle.MANIFEST_FILENAME)
    fields = {
...
def _resolve_inputs_hstr(args) -> typing.Dict[str, typing.Any]:
    if args.mode == "verify":
        images = hstr_runtime.enumerate_images(
            _require(args.suspect_path, "--suspect_path is required in verify mode")
        )
        if not images:
            raise SystemExit(f"error: no supported images found under {args.suspect_path}")
        return {"suspects": images}
    positives = hstr_runtime.enumerate_images(
        _require(args.positive_path, f"--positive_path is required in {args.mode} mode")
    )
    negatives = hstr_runtime.enumerate_images(
        _require(args.negative_path, f"--negative_path is required in {args.mode} mode")
    )
    if not positives or not negatives:
        raise SystemExit("error: both cohorts must contain at least one image")
    pairing = hstr_runtime.resolve_pairing(positives, negatives, args.pair_manifest)
    if args.mode == "paper_eval" and not pairing["paired"] and not args.allow_unmatched_cohorts:
        raise SystemExit(
...
def _score_hstr_cohort(provider, pipe_provider, image_paths, threshold_info, role: str):
    ...
    for index, image_path in enumerate(image_paths):
        row = hstr_runtime.score_image(provider, pipe_provider, image_path, threshold_info, image_index=index)
        row["cohort_role"] = role
        rows.append(row)
    return rows
...
def _main_hstr(args, argv) -> int:
    ...
    bundle_before = HstrBundle.load(bundle_dir).artifact_mtimes()
    inputs = _resolve_inputs_hstr(args)

    out_dir = Path(args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    gpu_info = hstr_runtime.gpu_preflight(DEVICE)
    print(f"[HSTR] device preflight: {gpu_info}", flush=True)
    pipe_provider = hstr_runtime.build_pipe_provider(args, DEVICE)
    provider = hstr_runtime.build_provider(
        args,
    ...
    if provider.bundle is None:
        raise HstrBundleError("HSTR verification must load state from an existing bundle")

    provenance = hstr_runtime.run_provenance(args, provider, pipe_provider)
    provenance["entrypoint"] = "eval_bench_wm/run_verify_watermark.py"
    provenance["mode"] = args.mode
    ...
    bundle_after = HstrBundle.load(bundle_dir).artifact_mtimes()
    if args.mode == "calibrate":
        changed = {k for k in set(bundle_before) | set(bundle_after) if bundle_before.get(k) != bundle_after.get(k)}
        unexpected = changed - {hstr_bundle.THRESHOLD_FILENAME}
        if unexpected:
            raise HstrBundleError(f"calibration modified bundle artifacts it must not touch: {sorted(unexpected)}")
    elif bundle_before != bundle_after:
        raise HstrBundleError("the HSTR bundle was modified during verification; this must never happen")
    print(hstr_bundle.canonical_json(summary), flush=True)
    return 0
...
def _run_verify_hstr(args, provider, pipe_provider, out_dir: Path, provenance, inputs) -> dict:
    ...
    hstr_bundle.write_jsonl(out_dir / "results.jsonl", rows)
    hstr_runtime.write_csv(out_dir / "results.csv", rows)
    ...
    (out_dir / "summary.json").write_text(hstr_bundle.canonical_json(summary) + "\n", encoding="utf-8")
    return summary
...
def _run_cohort_hstr(args, provider, pipe_provider, out_dir: Path, provenance, inputs) -> dict:
    ...
    if failed:
        hstr_bundle.write_jsonl(out_dir / "results.jsonl", rows)
        raise HstrBundleError(f"{len(failed)} HSTR cohort image(s) failed to score; first error: {failed[0]['error']}")
    ...
    roc = hstr_runtime.official_roc(positive_scores, negative_scores, args.hstr_target_fpr)
    ...
    summary.update({
        ...
        "positive_cohort_sha256": hstr_bundle.canonical_sha256({"images": [hstr_bundle.sha256_file(p) for p in inputs["positives"]]}),
        "negative_cohort_sha256": hstr_bundle.canonical_sha256({"images": [hstr_bundle.sha256_file(p) for p in inputs["negatives"]]}),
    ...
    if args.mode == "calibrate":
        artifact = hstr_bundle.build_threshold_artifact(
            threshold=roc["threshold"],
            binding=provider.bundle.binding_config(),
            target_fpr=roc["target_fpr"],
            empirical_fpr=roc["empirical_fpr"],
            empirical_tpr=roc["empirical_tpr"],
            roc_auc=roc["roc_auc"],
