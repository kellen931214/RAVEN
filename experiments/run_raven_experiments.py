#!/usr/bin/env python
"""Run RAVEN over an image directory with GPU auto-selection support."""
...
def main() -> None:
    ...
    try:
        pipe = RavenPipeline(model_id=args.model_id, device=args.device, dtype=args.dtype)
        ...
        for index, image_path in enumerate(iter_image_files(args.input_dir)):
            ...
            expected_output = item_dir / ("final_color_corrected.png" if args.color_transfer else "final.png")
            if expected_output.exists():
                skipped += 1
                print(f"Skipping completed RAVEN output {expected_output}", flush=True)
                continue
            try:
        ...
        write_experiment_records(
            ...
            utc_timestamp(),
            status,
            extra_summary={"num_processed": processed, "num_skipped": skipped, "num_failures": len(failures), "failed_files": failures},
        )
