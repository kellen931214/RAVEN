...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/raven_eval_mscoco.log"
...
printf '[%s] RAVEN eval full: RAM after\n' "$(date -Iseconds)"
free -h
printf '[%s] RAVEN eval full: GPU after\n' "$(date -Iseconds)"
nvidia-smi
